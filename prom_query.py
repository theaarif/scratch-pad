...
def run_query(q, base_url, cookie=None):
    ...
    r = requests.get(base_url + "/api/v1/query", cookies=cookie, headers=HEADERS, params={
        "query": q,
        "time": NOW
    })
...
def fetch_service_metrics(config):
    ...
    for name, tup in QUERIES.items():
        ...
        for result in run_query(query, config["base_url"], cookie=config["cookie"]):
            service_name = result["metric"]["service"]
...
def main():
    ...
    outer['Subject'] = "Weekly SLO Report"
    outer['From'] = config["email_from"]
    outer['To'] = config["email_to"]

    outer.attach(MIMEText("\n".join(plain_text(services)), _subtype="plain"))
    outer.attach(MIMEText("\n".join(html(services)), _subtype="html"))

    with smtplib.SMTP_SSL(config["smtp_host"]) as s:
        s.login(config["smtp_user"], config["smtp_pass"])
        s.sendmail(config["email_from"], config["email_to"], outer.as_string())
